  ...
  const particlesGeometry = useMemo(() => {
    ...
    try {
        if (!earthTexture || !earthTexture.image) {
            console.warn("Earth texture not loaded yet");
        } else {
            const img = earthTexture.image;
            if (img && img.width > 0 && img.complete) {
    ...
    // Ensure we have at least some particles (fallback if texture loading failed)
    if (positions.length === 0) {
      // Fallback: create particles randomly distributed
  ...
  useFrame((state, delta) => {
    try {
      ...
      if (hoverState.current.active) {
          ...
          if (surfaceRef.current && state.camera) {
      ...
      if (surfaceRef.current?.material?.uniforms) {
